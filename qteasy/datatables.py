...
DataSource：定义了所有的数据表，每种数据类型都存储在一个数据表中，数据表的结构由 TABLE_MASTERS 与 TABLE_SCHEMA 定义
...
完整的数据结构由两个字典来定义：
TABLE_MASTERS:          定义数据表的基本属性和下载 API 来源（目前仅包括 tushare，未来会添加其他 API）
TABLE_SCHEMA:           定义各表结构名称对应的列名、数据类型、主键与列说明（与 TABLE_MASTERS 首列 schema 键对应）


1, TABLE_MASTERS

table source mapping 定义了一张数据表的基本属性以及数据来源：
table_name(key):            数据表的名称（主键）自定义表名称不能与内置表名称重复
---------------------------------------------------------------------------------------------------------
1, schema:                  数据表的结构名称，根据该名称在 TABLE_SCHEMA 中可以查到表格包含的所有列、主键、数据类
                            型和详情描述
                            数据表的数据结构存储在不同的结构定义中，许多表可共享同一套结构名称
...
2, TABLE_SCHEMA:
Table schema 字典定义了各结构名称对应的数据表列布局：
table_schema_name:          数据结构名称（主键，与 TABLE_MASTERS[*][schema] 一致）
---------------------------------------------------------------------------------------------------------
columns:                    数据列名称
...
TABLE_SCHEMA = {
    ...
    'us_balance':
        {'columns':    ['ts_code', 'trade_date', 'period', 'filing_date', 'fiscal_year',
                        ...
                        'tax_assets', 'other_non_current_assets', 'total_non_current_assets',
                        'total_assets',
                        'accounts_payable', 'st_debt', 'deferred_revenue',
                        'other_current_liab', 'total_current_liab',
                        'lt_debt', 'other_non_current_liab', 'total_non_current_liab',
                        'total_liab',
         ...
         'remarks':    ['股票代码', '财报截止日', '报告类型(Q/Y)', '提交日期', '财年',
                        ...
                        '固定资产净值', '商誉', '无形资产', '长期投资',
                        '递延税资产', '其他非流动资产', '非流动资产合计',
                        '总资产',
                        '应付账款', '短期借款', '递延收入',
                        '其他流动负债', '流动负债合计',
                        '长期借款', '其他非流动负债', '非流动负债合计',
                        '负债合计',
                        '普通股', '留存收益', '累计其他综合收益', '股东权益',
                        '少数股东权益', '负债及股东权益合计',
                        '总债务', '净债务'],
         'prime_keys': [0, 1, 2]},
    ...
    'us_cashflow':
        {'columns':    ['ts_code', 'trade_date', 'period', 'filing_date', 'fiscal_year',
                        ...
                        'other_investing', 'cfi',
                        'net_debt_issuance', 'net_stock_issuance', 'dividends_paid',
                        'other_financing', 'cff',
